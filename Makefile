...
presubmit: verify-codegen verify verify-deadcode chart-lint ut-test ## Run all steps in the developer loop
...
.PHONY: help presubmit run ut-test require-project-id e2e-setup e2e-tests e2e-test e2e-teardown e2e-check-clean e2e-deploy coverage update update-pricing verify-codegen verify verify-deadcode image apply delete toolchain tidy download

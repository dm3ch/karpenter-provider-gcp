...
func fetchGCPWebPrices(ctx context.Context) (RegionPrices, error) {
	...
	for _, res := range results {
		...
		for region, machines := range res.prices {
			if out[region] == nil {
				out[region] = make(MachinePrices)
			}
			for machine, p := range machines {
				existing := out[region][machine]
				if p.OnDemand != 0 {
					existing.OnDemand = p.OnDemand
				}
				if p.Spot != 0 {
					existing.Spot = p.Spot
				}
				out[region][machine] = existing
			}
		}
	}
	return out, nil
...
func parsePricingTables(blocks []jsonNode, isSpot bool) RegionPrices {
	...
	for _, block := range blocks {
		...
		if result[region] == nil {
			result[region] = make(MachinePrices)
		}
		maps.Copy(result[region], parseDataRows(block, priceCol, isSpot))
	}
	return result
}
...
func parseDataRows(block jsonNode, priceCol int, isSpot bool) MachinePrices {
	out := make(MachinePrices)
